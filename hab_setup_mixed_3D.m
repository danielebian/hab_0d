 function hab = hab_setup_mixed_3D(hab,varargin)
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 SetUp.EndTime = 11*365*24;	% Duration of batch culture (hours)
 ...
 SetUp.iFlow = 3;	% 1: constant flow
                        % 2: idealized cycle
                        % 3: 3D BEC
 SetUp.Flow0 =200;  		% (iFlow=1) Rate of water flow at base of ML (m/y) (typically 100-1000?)
 ...
 case 4
    ...
    nn=length(Temp0)/12;
    years=SetUp.EndTime/(365*24);
    if nn~=1 & nn~=years
        error(['you need to define either 1 or ',num2str(max([years,nn])), ' year of data'])
    end
    
    time=reshape(SetUp.time,365*24*(years/nn),nn);
    Temp0=reshape(Temp0,12,nn);
    int_mode = 'pchip';
    for i=1:nn        
        aux(:,i) = interpolate_annual_cycle_to_model(time0,Temp0(:,i)',time(:,i)',int_mode);
    end
    SetUp.Env.Temp=reshape(aux,1,length(SetUp.time));       
...
error(['you need to define either 1 or ',num2str(max([years,nn])), ' year of data'])
    ...
    time=reshape(SetUp.time,365*24*(years/nn),nn);
    PAR0=reshape(PAR0,12,nn);
    int_mode = 'pchip';
    for i=1:nn        
        aux(:,i) = interpolate_annual_cycle_to_model(time0,PAR0(:,i)',time(:,i)',int_mode);
    end
    SetUp.Env.PAR=reshape(aux,1,length(SetUp.time));       
 otherwise
...
error(['you need to define either 1 or ',num2str(max([years,nn])), ' year of data'])
    ...
    time=reshape(SetUp.time,365*24*(years/nn),nn);
    MLD0=reshape(MLD0,12,nn);
    int_mode = 'pchip';
    for i=1:nn        
        aux(:,i) = interpolate_annual_cycle_to_model(time0,MLD0(:,i)',time(:,i)',int_mode);
    end
    SetUp.Env.MLD=reshape(aux,1,length(SetUp.time));       
 otherwise
 ...
 case 1
    ...
    for indv=1:nvar
        ...
        hab.BioPar.([hab.BioPar.varnames{indv} '_in'])=InVar* ones(1,SetUp.ntime);
...
error(['you need to define either 1 or ',num2str(max([years,nn])), ' year of data'])
    ...
    time=reshape(SetUp.time,365*24*(years/nn),nn);
    Flow0=reshape(Flow0,12,nn);
    int_mode = 'pchip';
    for i=1:nn        
        aux(:,i) = interpolate_annual_cycle_to_model(time0,Flow0(:,i)',time(:,i)',int_mode);
    end
    SetUp.Env.Flow=reshape(aux,1,length(SetUp.time));       
    ...
    for indv=1:nvar
        ...
        if length(InVar)==1
            InVar=ones(1,12*nn)*InVar;
        end
        
        InVar=reshape(InVar,12,nn);
        for i=1:nn
            aux(:,i) = interpolate_annual_cycle_to_model(time0,InVar(:,i)',time(:,i)',int_mode);
        end
        hab.BioPar.([hab.BioPar.varnames{indv} '_in'])=reshape(aux,1,length(SetUp.time));
