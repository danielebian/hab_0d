function extract_forcing_total(in,out)

%Function to get the forcings for all years
...
type='MONTHLY' % or 'clim'. Note, this is the type of output and the name of the folder.
simu='usw42'; % type/name of simulation. usw42=4km, ussw1=1km,
year=num2str(yr); % if you chose 'clim', then you have to put '_1997_2007'
...
for kk=1:12
    ...
    for indv=1:length(name3d)
        ...
        auz=indz~=0; %only when indz=0, auz=0 and the multiplication below is just the left size.
        for i=1:length(indz)
    ...
    for indv=1:length(name3d)-3
        ...
        aux=[(name3d{indv}),'_in'];
        var1d.(aux)(yr_inx) = N_in;
        
    end
    var1d.flow_in(yr_inx)=(T_in/(nansum(A5(:))))*60*60*24*365; %Total flow IN assumed as a consequence of upwelling [m/year]
    ...
    for indv=1:length(names)
        try
            ...
            var1d.(names{indv})(yr_inx) = nanmean(aux(:));
        catch
            var1d.(names{indv})(yr_inx) = NaN;
        end
        clear aux
    end
...
clearvars -except var1d yS yN xW xE work_path simu type year yr yr_inx in out

end

name3d=[work_path,simu,'_',type,'_',num2str(in),'_',num2str(out),'_lat_',num2str(yS),'_',num2str(yN),'_lon_',num2str(xW),'_',num2str(xE),'.mat'];
